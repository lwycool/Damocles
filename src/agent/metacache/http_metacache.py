...
# This is the HTTP metacache.

from src.agent.lib import urlbucket
from src.agent.metacache import metacache

class HttpMetaCache(metacache.MetaCache):

    def __init__(self, blacklist_source_session):
        super(HttpMetaCache, self).__init__(blacklist_source_session)
        # This dictionary (hashmap) consists
        # of the protocol://fqdn:<port> as the key
        # and a trie of all blacklisted urls as the
        # value.
        self.metacache = {}
        self.protocol = "http"
    ...
    def create_fqdn_bucket(self, fqdn):
        ...
        if fqdn_key in self.metacache:
            ...
            for row in blacklisted_urls_for_fqdn_key:
                ...
                print "url in bucket is: " + url_in_bucket
